...
const userSchema = new mongoose.Schema({
   ...
   password: {
      ...
      minlength: [6, "password is too short , must be over 6 characters lo ng"],
      maxlength: [10, "password too long , must be >= 10"],
      select: false
   },

   passwordConfirm: { // the same as the password needs to be 
      type: String,
      ...
      validate: {
         ...
         validator: function (passwordConfirm) { // current element : passwordConfirm
...
// userSchema.pre('save', async function (next) { // ! we use the pre('save') to encrypt is between the moment that we receive the data and the moment the data is stored in the database
...
userSchema.methods.correctPassword = async function (candidatePassword, userPassword) {
   ...
   // * the password the user passes in the body : candidate password : original password
   // * the password that is stored in the database : userPassword : hashed
   // ^ this.password won't work cuz we put select : false in the password field 
...
// userSchema.pre('save', async function (next) { //before a new document is saved in the database
...
userSchema.methods.changedPasswordAfter = function (JWTTimestamp) { //when the token was issued

   //*  by default false : means that user did not change his password 
   if (this.passwordChangedAt) { // & if passwordChangedAt exists only then we wanna do the comparaison
      // console.log(this.passwordChangedAt)
      const changedTimeStamp = parseInt(this.passwordChangedAt.getTime() / 1000, 10) //convert to ms and compare them
