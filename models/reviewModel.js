...
const reviewSchema = new mongoose.Schema({
   ...
   user:  // ! each review knows what user it belongs to :
   {
   ...
   room:  // ! each review knows what room it belongs to :
   {
      type: mongoose.Schema.ObjectId,
      ref: 'Room',
   },

   tour:  // ! each review knows what tour it belongs to :
   {
      type: mongoose.Schema.ObjectId,
      ref: 'Tour'
   }
...
// & preventing duplicated reviews
 // * each combination of room and user has to be unique
...
reviewSchema.statics.calcAverageRatingsTour = async function (tourId) {
   const stats = await this.aggregate([ // * points to the current model (Review)
   ...
   if (stats.length > 0) {
      await Tour.findByIdAndUpdate(tourId, //(tourId) by id
         { // fields to update
...
reviewSchema.post('save', function () { // ! post : after the review was saved in the db (was created)
...
reviewSchema.post('save', function () { // ! post : after the review was saved in the db
...
reviewSchema.post(/^findOneAnd/, async function () { // * after the review has been updated
