...
const roomSchema = new mongoose.Schema(
  {
    ...
    type: {
      ...
      enum: {
        ...
        message:
          " types are either : single, double, connecting rooms, twin, king, suite, presidantal suite, cabana, appartement ",
      },
    },
    ...
    locations: [
      // * we want to embed a document into another document so it needs to be an array // create a document inside a document but here location is not a document that why it must be an array that how embedding works // sub document
...
roomSchema.pre("save", function (next) {
  //! run before an actual event //save event just like in js
...
roomSchema.post(/^find/, function (docs, next) {
  //! we have access to all the document that were returned from the query //query already finished
...
//^ virtual properties:
roomSchema.virtual("durationWeeks").get(function () {
  //*not gonna be persisted in the database // *each time we get our data from the data base gonna be called //each time we get data from tha database we need to recall this virtual property so that it can be with the schema in the real life
...
roomSchema.virtual("reviews", {
  // * we want to populate the reviews field with the reviews that are referencing this room
