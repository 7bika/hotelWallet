...
const multerFilter = (req, file, cb) => {
  if (file.mimetype.startsWith("image")) {
    // ^ mimetype: 'image/jpeg' starts with image
    cb(null, true)
  } else {
...
exports.aliasTopRooms = (req, res, next) => {
  ...
  req.query.fields = "name,price,ratingsAverage,summary,type,imageCover"
...
exports.getMonthlyPlan = catchAsync(async (req, res, next) => {
  ...
  const plan = await Room.aggregate([
    ...
    },
    {
      $match: {
        startDates: {
          // ? in the start dates from the schema
    ...
    {
      $project: {
        // ! project the fields that we want to display //we give each of the field names  0 or 1
...
exports.getOurHotel = catchAsync(async (req, res, next) => {
  ...
  }) // * finds documents within a certain geometry
